...
class IllinoisICCScraper(GenericScraper[ILICCCaseData, ILICCFilingData]):
    ...
    def _get_driver(self):
        ...
        try:
            ...
            driver = webdriver.Chrome(options=options) # Assumes chromedriver is in PATH or managed by Selenium Manager
            driver.set_page_load_timeout(60)  # Set default timeout to 60 seconds
            return driver
        except Exception as e:
            print(f"Error initializing WebDriver: {e}")
            print("Ensure chromedriver is installed and in your PATH, or use webdriver-manager.")
            raise
    ...
    def universal_caselist_intermediate(
        ...
        if stop_at_case_identifier:
             print(f"Will stop scraping if identifier '{stop_at_case_identifier}' is found.")
             
        driver = self._get_driver()
        extracted_cases: List[Tuple[str, str]] = [] # List to store (URL, Number) tuples
        processed_case_urls = set() # To handle potential duplicates across pages if pagination glitches
        ...
        try:
            ...
            print(f"Waiting for initial case list items ({list_item_selector}) to load...")
            
            # Add better error handling for the initial page load
            try:
                wait.until(EC.presence_of_element_located((By.CSS_SELECTOR, list_item_selector)))
                print("Initial search results loaded.")
            except TimeoutException:
                print("Timeout waiting for search results to load. Checking if any content loaded...")
                ...
                try:
                    ...
                    if "Error" in driver.title or "Error" in body.text:
                        print("Error page detected. URL might be invalid or site is down.")
                    return {"error": "Timeout during search init", "extracted_cases": []}
                except:
                    print("Could not even find body element. Page likely didn't load at all.")
                    return {"error": "Failed to load search page", "extracted_cases": []}

            current_page = 1
            while True: # Pagination loop
                print(f"Extracting data from results page {current_page}...")
                time.sleep(1) 
                case_link_selector = "li.soi-icc-card-list-item h3 > a[href*='/docket/']"
                
                try:
                    wait.until(EC.presence_of_element_located((By.CSS_SELECTOR, case_link_selector)))
                    link_elements = driver.find_elements(By.CSS_SELECTOR, case_link_selector)
                    print(f"Found {len(link_elements)} case links on page {current_page}.")
                    ...
                    for link_tag_element in link_elements: # Loop through cases on page
                        try:
                            ...
                            if not relative_url or not case_number: continue
                                
                            case_url = f"{self.BASE_URL}{relative_url}" if relative_url.startswith('/') else relative_url
                            ...
                            if stop_at_case_identifier and identifier_to_check == stop_at_case_identifier:
                                print(f"Found stop case identifier: {identifier_to_check}. Stopping extraction.")
                            ...
                            if case_url not in processed_case_urls:
                             ...
                             print(f"Error processing a link element on page {current_page}: {e_inner}")
                             
                    print(f"Added {page_cases_added} new cases from page {current_page}.")
                    ...
                    if stop_processing:
                     ...
                     print(f"Warning: Timeout waiting for case links ({case_link_selector}) on page {current_page}. Might be empty or loading issue.")
                     break
                except Exception as e: # ...
                     print(f"Error finding or processing link elements on page {current_page}: {e}")
                     break
                ...
                try:
                    ...
                    if not next_buttons:
                        print("No 'Next' button found or it's disabled. End pagination.")
                        break
            ...
            print(f"Finished extraction. Total unique cases extracted: {len(extracted_cases)}")
            return {"extracted_cases": extracted_cases, "search_url": target_url}
        except TimeoutException: # ...
            print("Timeout occurred during initial navigation...")
            return {"error": "Timeout during search init", "html_on_error": driver.page_source if driver else None}
        except Exception as e: # ...
            print(f"An error occurred during universal_caselist_intermediate: {e}")
            return {"error": str(e), "html_on_error": driver.page_source if driver else None}
        finally: # ...
    ...
    def universal_caselist_from_intermediate(
        ...
        if intermediate.get("error"): 
            print(f"Skipping case conversion due to error in intermediate step: {intermediate['error']}")
            return []
        ...
        print(f"Converting {len(extracted_cases)} extracted tuples to ILICCCaseData objects...")
        for case_url, case_number in extracted_cases:
            try:
                ...
                print(f"Error creating ILICCCaseData for URL {case_url}, Number {case_number}: {e}")
    ...
    def filing_data_intermediate(self, data: ILICCCaseData) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                print(f"Warning: Timeout while loading docket sheet page: {docket_sheet_url}")
            ...
            try:
                ...
                if not doc_list:
                    ...
                    all_links = documents_soup.find_all('a', href=lambda href: href and '/documents/' in href)
                    if all_links:
                ...
                print(f"Found {len(intermediate_result['document_detail_urls'])} document URLs for {data.case_number}.")
            except TimeoutException:
            ...
            try:
                ...
                print(f"Warning: Timeout while loading service list page: {service_list_url}")
    ...
    def filing_data_from_intermediate(
        ...
        if "raw_docket_sheet_html" in intermediate:
            docket_soup = BeautifulSoup(intermediate["raw_docket_sheet_html"], 'html.parser')
        ...
        if "raw_service_list_html" in intermediate:
            service_soup = BeautifulSoup(intermediate["raw_service_list_html"], 'html.parser')
        ...
        if "raw_schedule_html" in intermediate:
            schedule_soup = BeautifulSoup(intermediate["raw_schedule_html"], 'html.parser')
        ...
        if "document_detail_urls" in intermediate and intermediate["document_detail_urls"]:
            ...
            try:
                ...
                for doc_url in document_urls:
                    try:
                        ...
                        doc_data = self._parse_document_details_from_soup(doc_soup, doc_url)
    ...
    def updated_cases_since_date_from_intermediate(
        ...
        for case in cases:
            if case.case_details and case.case_details.filing_date:
                try:
                    case_date = datetime.strptime(case.case_details.filing_date, "%m/%d/%Y").date()
    ...
    def into_generic_case_data(self, state_data: ILICCCaseData) -> GenericCase:
        ...
        return GenericCase(
            ...
            case_title=state_data.case_details.title if state_data.case_details else None,
            case_url=state_data.case_url,
            category=state_data.category,
            service_type=None,  # Not available in Illinois data
            judge_name=state_data.case_details.judge_names if state_data.case_details else None,
            filing_date=self._parse_date(state_data.case_details.filing_date) if state_data.case_details else None,
            case_status=state_data.case_details.status if state_data.case_details else None,
    ...
    def into_generic_filing_data(self, state_data: ILICCFilingData) -> GenericFiling:
        ...
        return GenericFiling(
            ...
            document_url=state_data.filing_documents[0].url if state_data.filing_documents else ""
        )
