...
def run_all_cases(scraper_info: ScraperInfoObject, years: list[int] | None):
    ...
    cases = get_all_caselist_raw_jsonified(scraper=scraper, base_path=base_path, year_list=years)
    
    with ThreadPoolExecutor(max_workers=10) as executor:
        futures = [executor.submit(process_case_jsonified, scraper, case, base_path) for case in cases]
...
def run_new_cases(scraper_info: ScraperInfoObject, after_date: str | None):
    ...
    logging.info(f"Running scraper for new cases for {scraper_info.id} after {after_date_dt.date()}")
    
    cases = get_new_caselist_since_date_jsonified(scraper=scraper, after_date=after_date_dt, base_path=base_path)
...
def run_test_single_docket(scraper_info: ScraperInfoObject):
    ...
    try:
        result = process_case_jsonified(scraper=scraper, case=case_json, base_path=base_path)
        logging.info(f"Successfully processed case: {result}")
    except Exception as e:
        logging.error(f"Error processing case: {e}")
...
def main():
    ...
    parser_all_cases = subparsers.add_parser("all-cases", help="Run the scraper for all cases.")
    parser_all_cases.add_argument("--scraper", required=True, choices=scraper_choices, help="The scraper to run.")
    parser_all_cases.add_argument("--years", help="A comma-separated list of years to scrape.")

    # Sub-parser for new-cases
    parser_new_cases = subparsers.add_parser("new-cases", help="Run the scraper for new cases.")
    parser_new_cases.add_argument("--scraper", required=True, choices=scraper_choices, help="The scraper to run.")
    parser_new_cases.add_argument("--after-date", help="The date to scrape new cases after (YYYY-MM-DD).")

    # Sub-parser for test-single-docket
    parser_test_single_docket = subparsers.add_parser("test-single-docket", help="Run the scraper for a single test docket.")
    parser_test_single_docket.add_argument("--scraper", required=True, choices=scraper_choices, help="The scraper to run.")
