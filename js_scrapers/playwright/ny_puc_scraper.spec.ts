...
class NyPucScraper implements Scraper {
    ...
    async getCaseList(): Promise<Partial<GenericCase>[]> {
        ...
        while (true) {
            ...
            await this.page.waitForLoadState("networkidle");
            const rows = await this.page.locator("#grdCaseMaster tr.gridrow, #grdCaseMaster tr.gridaltrow").all();
            ...
            const pagerButtons = await this.page.locator("#grdCaseMaster_pg td a").all();
    ...
    async getCaseDetails(caseData: Partial<GenericCase>): Promise<GenericCase> {
        ...
        const publicDocuments = await this.scrapeDocuments("#MainContent_grdPublicDocuments");
        const caseDocuments = await this.scrapeDocuments("#MainContent_grdCaseDocuments");
        ...
        const fullCaseData: GenericCase = {
            ...
            hearing_officer: "",
            closed_date: null,
            filings: [...publicDocuments, ...caseDocuments],
            case_parties: [],
            extra_metadata: {},
        } as GenericCase;
    ...
    private async scrapeDocuments(tableSelector: string): Promise<GenericFiling[]> {
        const documents: GenericFiling[] = [];
        while (true) {
            await this.page.waitForLoadState("networkidle");
            const docRows = await this.page.locator(`${tableSelector} tr.gridrow, ${tableSelector} tr.gridaltrow`).all();
            ...
            const pagerButtons = await this.page.locator(`${tableSelector}_pg td a`).all();
            if (pagerButtons.length > 1) {
                const nextButton = pagerButtons[pagerButtons.length - 1];
                const nextButtonText = await nextButton.innerText();
                if (nextButtonText === ">") {
