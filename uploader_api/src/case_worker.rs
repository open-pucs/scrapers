...
use chrono::Utc;
use mycorrhiza_common::s3_generic::cannonical_location::{download_openscrapers_object, upload_object};
...
#[derive(Clone,Default, Copy, Deserialize, Serialize)]
...
impl RawDocketUploadType {
    fn join_dockets(
        ...
        if uploaded.case_govid != original.case_govid {
            return anyhow!("Docket id's do not match for dockets that are being merged.");
        }
    ...
    async fn download_and_compare(
        ...
        docket_addres: &DocketAddress,
    ) -> anyhow::Result<RawGenericDocket> {
        let original_docket = download_openscrapers_object<RawGenericDocket>(s3_client, docket_addres).await?;
